...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ... (tus otras constantes para elementos del DOM se mantienen igual)
    const productGrid = document.getElementById('product-grid');
    const categoryNav = document.getElementById('category-nav');
    // ... (resto de constantes que ya tienes)
    ...
    let ebayTokenInMemory = null; // Token en memoria para la sesión actual
    let currentProductsData = [];

    const PRODUCT_CACHE_DURATION_MS = 24 * 60 * 60 * 1000;
    const TOKEN_CACHE_KEY = 'ebay_api_token_data_v2'; // Cambiar clave si la estructura del objeto cacheado cambia
    // Duración de caché del token un poco menos de 2 horas (120 min = 7200s)
    // 115 minutos = 6900 segundos = 6900000 ms
    const TOKEN_CACHE_DURATION_MS = 115 * 60 * 1000; 

    // ... (signalBaseUrl, simplexFullUrl y categories se mantienen como en tu última versión funcional)
    ...
    async function fetchEbayToken(forceRefresh = false) {
        ...
        try {
            ...
            ebayTokenInMemory = tokenData.access_token; // Actualizar token en memoria

            // Calcular duración de caché para el token nuevo
            // Asumimos que tu backend devuelve 'expires_in' en segundos
            const tokenValidDurationMs = tokenData.expires_in 
                ? (tokenData.expires_in * 1000 * 0.95) // Cachear por 95% de su vida útil
                : TOKEN_CACHE_DURATION_MS; // Fallback a 115 minutos
            ...
            ebayTokenInMemory = null; // Invalidar token en memoria
            return null;
        }
    }

    async function fetchProducts(query, categoryName, limit = 50, offset = 0) { // LÍMITE CAMBIADO A 50
        ...
        // Obtener el token. Si hay un problema con el token, esta función ya mostrará un error.
        const token = await fetchEbayToken(); 
        if (!token) {
            // fetchEbayToken ya habrá llamado a showError si falla críticamente.
            // Si solo devuelve null porque no hay token en memoria/cache y debe forzar,
            // la siguiente llamada podría fallar, o si el token es inválido, eBay lo rechazará.
        ...
        try {
            ...
            if (response.status === 401) { // Unauthorized - Específicamente para token inválido
                ...
                // Forzar refresco del token y reintentar la búsqueda UNA VEZ.
                ...
                if (newToken) {
                    ...
                    if (!retryResponse.ok) {
                        ...
                        try{const eD=await retryResponse.json();eMsg=eD.error||eD.message||eMsg;}catch(e){}
                        throw new Error(eMsg);
                    }
                    // Continuar con el procesamiento de retryResponse.json()
                    const data = await retryResponse.json();
                    // ... (resto de la lógica de procesamiento de 'data' como abajo)
                    let itemsArray = [];
                    if (data) {
                        ...
                        else console.warn(`[Frontend] Para "${categoryName}" (tras reintento), respuesta NO esperada.`, data);
                    } else console.warn(`[Frontend] Para "${categoryName}" (tras reintento), respuesta API nula.`);
    ...
    // ... (displayProducts, populateCategories, y todas las funciones de modales y sus listeners
    //      se mantienen IGUALES a la última versión completa que te pasé.
    //      Asegúrate de que displayProducts use la aleatorización y la lógica de imagen HD)
    function shuffleArray(array) { /* ... (función de aleatorizar) ... */ 
    ...
    function displayProducts(itemsToDisplay, categoryNameForMsg, randomizeOrder = false) {
        ...
        finalItems.forEach((item, index) => {
            ...
            if(item.image?.imageUrl)sourceImageUrl=item.image.imageUrl;else if(item.thumbnailImages?.[0]?.imageUrl)sourceImageUrl=item.thumbnailImages[0].imageUrl;else if(item.additionalImages?.[0]?.imageUrl)sourceImageUrl=item.additionalImages[0].imageUrl;
            if(sourceImageUrl){bestImageUrl=sourceImageUrl;if(bestImageUrl.includes('s-l'))bestImageUrl=bestImageUrl.replace(/s-l\d+/ig,'s-l1600');}
            const imageUrl=bestImageUrl;const title=item.title||'Producto';const price=item.price?.value?parseFloat(item.price.value):0;const currency=item.price?.currency||'USD';const itemId=item.itemId||`temp_${Date.now()}_${index}`;
            const rating=Math.floor(Math.random()*3)+3;let starsHTML='';for(let i=1;i<=5;i++)starsHTML+=`<i class="${i<=rating?'fas':'far'} fa-star"></i>`;
            card.innerHTML = `
            ...
            card.addEventListener('click',(e)=>{if(e.target.classList.contains('consult-button')){openConsultationModal(e.target.dataset.itemTitle,e.target.dataset.itemId);}else{const pIdx=parseInt(e.currentTarget.dataset.index,10);const pData=currentProductsData[pIdx];if(pData)openProductDetailsModal(pData);}});
        });
    }
    
    function populateCategories(){categories.forEach((cat,idx)=>{const btn=document.createElement('button');btn.textContent=cat.name;btn.addEventListener('click',(e)=>{document.querySelectorAll('#category-nav button').forEach(b=>b.classList.remove('active'));e.currentTarget.classList.add('active');fetchProducts(cat.query,cat.name);});categoryNav.appendChild(btn);if(idx===0){btn.classList.add('active');fetchProducts(cat.query,cat.name);}});}
    ...
    function openConsultationModal(productTitle,productItemId){consultationProductNameModal.textContent=productTitle;const msg=`Hola, estoy interesado/a en el producto: "${productTitle}" (ID: ${productItemId}). Me gustaría saber más sobre cómo adquirirlo con BTCPY1.`;copyPasteMessageText.value=msg;signalContactLink.href=`${signalBaseUrl}?message=${encodeURIComponent(msg)}`;simplexContactLink.href=simplexFullUrl;consultationModal.classList.add('active');document.body.style.overflow='hidden';}
    function openProductDetailsModal(productData){let bestModalImgUrl='https://via.placeholder.com/300x350.png?text=No+Image';let srcModalImgUrl=null;if(productData.image?.imageUrl)srcModalImgUrl=productData.image.imageUrl;else if(productData.thumbnailImages?.[0]?.imageUrl)srcModalImgUrl=productData.thumbnailImages[0].imageUrl;else if(productData.additionalImages?.[0]?.imageUrl)srcModalImgUrl=productData.additionalImages[0].imageUrl;if(srcModalImgUrl){bestModalImgUrl=srcModalImgUrl;if(bestModalImgUrl.includes('s-l'))bestModalImgUrl=bestModalImgUrl.replace(/s-l\d+/ig,'s-l1600');}
    modalProductImage.src=bestModalImgUrl;modalProductImage.onerror=function(){this.onerror=null;this.src=srcModalImgUrl||'https://via.placeholder.com/300x350.png?text=Error+Img';};modalProductTitle.textContent=productData.title||'Producto';const rating=Math.floor(Math.random()*3)+3;let starsHTML='';for(let i=1;i<=5;i++)starsHTML+=`<i class="${i<=rating?'fas':'far'} fa-star"></i>`;modalProductRating.innerHTML=starsHTML;const price=productData.price?.value?parseFloat(productData.price.value):0;modalProductPriceUSD.textContent=`${price.toFixed(2)} ${productData.price?.currency||'USD'}`;modalProductPriceBTC.textContent=`${price.toFixed(2)} BTCPY1`;modalProductSpecsList.innerHTML='';const specs=productData.localizedAspects||[];let specsCount=0;if(specs.length>0){specs.forEach(spec=>{if(specsCount<8){const li=document.createElement('li');li.innerHTML=`<strong>${spec.name||'Caract.'}:</strong> ${spec.value||'N/E'}`;modalProductSpecsList.appendChild(li);specsCount++;}});}
    if(productData.condition&&!specs.some(s=>s.name?.toLowerCase()==='condition')){const li=document.createElement('li');li.innerHTML=`<strong>Condición:</strong> ${productData.condition}`;modalProductSpecsList.appendChild(li);specsCount++;}
    if(specsCount===0)modalProductSpecsList.innerHTML='<li>No hay especificaciones.</li>';modalProductDescription.textContent=productData.shortDescription||'Descripción no disponible. Visite eBay o consulte.';modalEbayLink.href=productData.itemWebUrl||'#';modalConsultButton.onclick=()=>{closeModal(productDetailsModal);openConsultationModal(productData.title,productData.itemId||`temp_${Date.now()}`);};productDetailsModal.classList.add('active');document.body.style.overflow='hidden';}
    function closeModal(modalEl){if(modalEl){modalEl.classList.remove('active');if(!document.querySelector('.modal.active'))document.body.style.overflow='auto';}}
    document.querySelectorAll('.close-modal-button').forEach(btn=>btn.addEventListener('click',(e)=>closeModal(e.target.closest('.modal'))));window.addEventListener('click',(e)=>{if(e.target.classList.contains('modal'))closeModal(e.target);});window.addEventListener('keydown',(e)=>{if(e.key==='Escape')document.querySelectorAll('.modal.active').forEach(closeModal);});
